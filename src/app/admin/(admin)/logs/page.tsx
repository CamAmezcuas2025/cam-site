...
export default function LogsPage() {
  ...
  async function fetchLogs() {
    const { data: rawLogs, error } = await supabase
      .from("hours_logs")
      .select("id, class_name, date, hours, user_id, created_at")
    ...
    const userIds = rawLogs?.map((log) => log.user_id) || [];
    if (userIds.length > 0) {
      ...
      const profileMap = new Map(logProfiles?.map((p) => [p.id, p]) || []);

      const flattened = rawLogs.map((item: any) => ({
        ...item,
        duration: item.hours,
  ...
  async function fetchClasses() {
    const staticClasses = [
      { id: "1", name: "Boxeo", instructor: "Coach A" },
      { id: "2", name: "Jiu Jitsu", instructor: "Coach B" },
    ];
  ...
  async function handleDelete(logId: string) {
    ...
    await supabase.from("hours_logs").delete().eq("id", logId);
