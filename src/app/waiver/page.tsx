...
import { createClientSupabaseClient } from "../lib/clientSupabaseClient";
...
export default function WaiverPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.accepted_esign_law) {
      setResult({ error: "Debes aceptar la validez legal de la firma electrónica." });
      setSubmitting(false);
      return;
    }
    ...
    if (sigCanvasRef.current && !sigCanvasRef.current.isEmpty()) {
      ...
      const fileName = `${profile?.id || 'anon'}/waiver-${Date.now()}.png`;
    ...
    const body = {
      ...
      signature_url,
      gym_owner_1: OWNERS.split(",")[0].trim(),
      gym_owner_2: OWNERS.split(",")[1]?.trim() || OWNERS.split(",")[0].trim(),
      allowed_marketing: form.allow_image_rights, // Map to marketing if needed; adjust if separate
      user_id: profile?.id,
      ...(form.is_minor
        ? {
    ...
    return (
      <div className="max-w-xl mx-auto my-10 p-6 bg-black/70 rounded-xl text-white text-center">
        ...
        <button
          ...
          className="mt-4 px-6 py-2 bg-brand-blue text-white rounded-lg hover:bg-brand-red transition-colors"
        >
          Volver al Perfil
        </button>
      </div>
