...
export async function POST(req: NextRequest) {
  ...
  // Upsert to profiles for totals/streak (calc weekly/monthly/total, streak)
  ...
  const totalHours = classLogs?.reduce((sum, log) => sum + log.hours, 0) || 0;
  ...
  const streak = recentLogs?.length || 0; // Placeholder—count consecutive
  ...
  if (classIndex > -1) {
    ...
    classProgress.push({ name: className, weeklyHours: 0, monthlyHours: 0, totalHours });
  }
